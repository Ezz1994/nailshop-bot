...
import { toJordanYMD } from "./utils/dates";
import { http } from "@/lib/http";            // ✅ authenticated axios client
import axios from "axios";                    // for axios.isCancel / isAxiosError
...
// --- normalizeBooking helper, now handles missing end_at ---
const normalizeBooking = (raw: RawBooking): Booking => ({
  ...
  services: Array.isArray(raw.booking_services)
  ...
  total_duration: typeof raw.total_duration === "number" ? raw.total_duration : 0,
});

// Convert RawService → BookingService
function mapRawService(s: RawService): BookingService {
  ...
  const detail = {
    id: s.service?.service_id ?? sid,
    name_en: s.service?.name_en ?? s.name_en ?? "",
    name_ar: s.service?.name_ar ?? s.name_ar ?? "",
    price: s.service?.price_jd ?? s.price_jd ?? s.price ?? 0,
    duration_min: s.service?.duration_min ?? s.duration_min ?? 0,
    staff_level: s.service?.staff_level ?? s.staff_level ?? "",
  };
  ...
  return {
    ...
    price_jd: detail.price,
    staff_level: detail.staff_level,
    service: detail,
  };
}
...
const DayView = ({
    ...
    const run = async () => {
      try {
        ...
        const res = await http.get<{ bookings: RawBooking[] }>("/bookings/day", {
          params: { date: dayISO },
          signal: controller.signal,                    // no 'any'
        });
  ...
  // If the parent didn't give us bookings, fetch them; otherwise keep parent’s data
  useEffect(() => {
  ...
  const handleStatusUpdateWithDrawer = async (bookingId: string, status: BookingStatus) => {
    await onStatusUpdate(bookingId, status); // WeeklyCalendar handles the API (already authenticated)
    if (status === "cancelled") {
      setIsDrawerOpen(false);
      setSelectedBooking(null);
    }
    setLocalRefresh((k) => k + 1); // force day refetch
  };

  useEffect(() => {
    if (bookings.length > 0) console.log("DayView bookings:", bookings);
  }, [bookings]);
    ...
    <div className="space-y-6">
                  ...
                  <div className="flex justify-between items-start">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-gray-600">
                            {(booking.services ?? []).map((bs) => bs.name_en || "").join(", ")}
                          </span>
                        </div>
                    ...
                    <div className="text-right">
                      <div className="text-lg font-bold text-primary">
                        {booking.services.reduce((sum, s) => sum + (s.price_jd ?? 0), 0)} JD
                      </div>
                      <div className="text-sm text-gray-500">
                        {booking.services.reduce((sum, s) => sum + (s.duration_min ?? 0), 0)} min
                      </div>
                    </div>
                  </div>
                </CardContent>
      ...
      <WalkinModal
        ...
        onSubmit={() => {
          ...
          toast({ title: "Booking Added", description: "Walk-in created.", duration: 2500 });
        }}
      />
    </div>
  );
